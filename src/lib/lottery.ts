...
function _sha256(message: Uint8Array): Uint8Array {
  ...
  for (let j: number = 0; j < wordLength; j += 16) {
    ...
    let [a0, a1, a2, a3, a4, a5, a6, a7]: number[] = [
      h0,
      h1,
      h2,
      h3,
...
function _sel(rng: RNG, players: WeightedPlayers): string {
  ...
  return _srt(Object.keys(players))
    .map((player: string): string | null => {
      ...
      return current - players[player] <= selected && selected < current
        ? player
        : null;
    })
...
function simulate(
  ...
  const counts: WeightedPlayers = Object.fromEntries(
  ...
  const { max, min, idx }: { max: number; min: number; idx: number } = buckets
    .map(
      (
        ...
        idx: number,
      ): { max: number; min: number; idx: number } | null =>
        min < power && power <= max ? { max, min, idx } : null,
    )
    .filter(
      (
        x: { max: number; min: number; idx: number } | null,
      ): x is { max: number; min: number; idx: number } => null !== x,
    )[0];
